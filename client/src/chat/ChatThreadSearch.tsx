...
interface ChatThreadSearchProps {
  open: boolean
  onClose: () => void
  onJumpToMessage: (messageId: number | string) => void
  threadType: 'dm' | 'group'
...
export default function ChatThreadSearch({
  ...
  const getSenderLabel = (result: SearchResult) => {
    if (threadType === 'dm') return result.sent ? t('chat.you', 'You') : ''
    return result.sender || result.sender_username || ''
  }
    ...
    <>
      <style>{`
        ...
        @keyframes searchHighlight {
          0% { background-color: rgba(77, 182, 172, 0.3); }
      ...
      >
        <div className="flex flex-col w-full max-w-lg mx-auto mt-[env(safe-area-inset-top,0px)] h-full max-h-[80vh] my-auto rounded-xl overflow-hidden bg-[#111] border border-white/10">
          ...
          {/* Count badge */}
          {searched && total > 0 && (
            <div className="px-3 py-1.5 text-xs text-[#4db6ac] border-b border-white/5">
              {total}{' '}
              {total === 1
                ? t('chat.match', 'match')
          ...
          <div className="flex-1 overflow-y-auto">
                ...
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    {getSenderLabel(result) && (
                      <div className="text-xs text-[#4db6ac] font-medium mb-0.5 truncate">
                        {getSenderLabel(result)}
                      </div>
                    )}
            ...
            {hasMore && (
              <button
                type="button"
                className="w-full py-3 text-sm text-[#4db6ac] hover:bg-white/5 transition-colors"
