...
export default function CommunityCalendar(){
    ...
    async function load(){
      ...
      try{
        ...
        if (j?.success && Array.isArray(j.events)){
          // Filter events by community_id if present in payload description or we have community_id in event (if backend filled it)
  ...
  async function rsvp(eventId:number, response:'going'|'maybe'|'not_going'){
    try{
      ...
      // Fallback: just optimistic update counts locally
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-2xl mx-auto p-3 space-y-4">
        <form className="rounded-xl border border-white/10 p-3 bg-white/5" onSubmit={(e)=> { e.preventDefault(); createEvent(new FormData(e.currentTarget)) }}>
          <div className="text-sm font-medium mb-2">Create Event</div>
          <div className="grid grid-cols-2 gap-2">
            <input name="title" placeholder="Title" className="col-span-2 rounded-md bg-black border border-white/10 px-3 py-2 text-[16px] focus:border-teal-400/70 outline-none" required />
            <input name="date" type="date" className="rounded-md bg-black border border-white/10 px-3 py-2 text-[16px] focus:border-teal-400/70 outline-none" required />
            <input name="end_date" type="date" className="rounded-md bg-black border border-white/10 px-3 py-2 text-[16px] focus:border-teal-400/70 outline-none" />
            <input name="start_time" type="time" className="rounded-md bg-black border border-white/10 px-3 py-2 text-[16px] focus:border-teal-400/70 outline-none" />
