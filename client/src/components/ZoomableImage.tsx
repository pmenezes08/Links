...
export default function ZoomableImage({ src, alt = 'image', className = '', maxScale = 3, onRequestClose }: ZoomableImageProps) {
  ...
  return (
    <div
      ...
      onTouchStart={(e) => { if ('PointerEvent' in window) return; if (e.touches.length===2){ e.preventDefault(); const a=e.touches[0], b=e.touches[1]; pinchStartDistance.current = Math.hypot(b.clientX-a.clientX, b.clientY-a.clientY); pinchStartScale.current = scale } }}
      onTouchMove={(e) => { if ('PointerEvent' in window) return; if (e.touches.length===2 && pinchStartDistance.current){ e.preventDefault(); const a=e.touches[0], b=e.touches[1]; const dist=Math.hypot(b.clientX-a.clientX, b.clientY-a.clientY); const factor=dist/(pinchStartDistance.current||1); const nextScale=clamp(pinchStartScale.current*factor, minScale, maxScale); const rect=containerRef.current?.getBoundingClientRect(); if(rect){ const cx=((a.clientX+b.clientX)/2 - rect.left - rect.width/2); const cy=((a.clientY+b.clientY)/2 - rect.top - rect.height/2); const scaleRatio=nextScale/scale; const nextX=cx - (cx - translate.x)*scaleRatio; const nextY=cy - (cy - translate.y)*scaleRatio; const {maxX,maxY}=getBounds(nextScale); setTranslate({ x: clamp(nextX, -maxX, maxX), y: clamp(nextY, -maxY, maxY) }) } setScale(nextScale) } }}
      onTouchEnd={(e)=> { if ('PointerEvent' in window) return; if (e.touches.length<2) { pinchStartDistance.current=null } }}
      role="img"
      aria-label={alt}
    >
      <img
