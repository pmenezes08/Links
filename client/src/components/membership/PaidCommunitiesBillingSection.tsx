...
export default function PaidCommunitiesBillingSection() {
  ...
  if (loading) {
    return (
      <p className="text-sm text-c-text-tertiary">{t('billing.paid_communities_loading')}</p>
    )
  }
  if (err) {
    return (
  ...
  if (rows.length === 0) {
    return (
      <p className="text-sm text-c-text-tertiary">{t('billing.paid_communities_empty')}</p>
    )
  }
  ...
  return (
    <section className="rounded-xl border border-c-border bg-c-hover-bg overflow-hidden">
      <div className="px-4 py-3 border-b border-c-border">
        <h4 className="text-sm font-semibold text-c-text-primary">{t('billing.paid_communities_title')}</h4>
        <p className="mt-1 text-xs text-c-text-tertiary">{t(subtitleKey)}</p>
        {notice && (
          ...
          return (
            ...
            >
              <div className="min-w-0">
                <div className="font-medium text-c-text-primary truncate">{row.name}</div>
                <div className="mt-1.5 flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full border border-c-border bg-c-hover-bg px-2 py-0.5 text-[11px] font-medium text-c-text-tertiary">
              ...
              <button
                ...
                className="shrink-0 rounded-full border border-c-border px-4 py-2 text-xs font-semibold text-c-text-primary hover:bg-c-hover-bg disabled:opacity-50 transition"
              >
                {busy
