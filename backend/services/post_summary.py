"""Steve post summaries — authorized, gated, and logged.
...
authorization). This service restores all three:

* requester must be a member of the post's community (non-enumerating 404);
* the call is gated via ``check_steve_access`` with ``community_id`` so
  in-community calls draw from the Steve Community Package pool (blocked
  calls get their ``log_block`` row inside the gate);
...
def generate_post_summary(username: str, post_id: int) -> Tuple[Dict[str, Any], int]:
    """Authorize, gate, call Grok, log, and return the JSON body + status."""
    if not XAI_API_KEY:
