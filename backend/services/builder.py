...
_SYSTEM_PROMPT = (
    ...
    "a rating prompt, and Play again. THEREFORE, when CPoint is present, DO NOT build your OWN game-over/results "
    "screen, score-save button, leaderboard, rating UI, or play-again/replay button — they overlap and collide with "
    "the native one (the #1 bug to avoid). Just end the run by calling `CPoint.gameOver({score})` and stop (a brief "
    "in-canvas 'Game Over' flash with no buttons is fine). ONLY when CPoint is ABSENT, show your own simple end screen. "
...
_CONVERSE_BASE = (
    ...
    "counts. AND — IMPORTANT — WHEN YOU BUILD, you RESEARCH REAL FACTS FROM THE WEB and bake them in: real golf course "
    ...
    "build-time web research, vetted public data, save, leaderboards) DO work — never refuse those. If the user asks for "
