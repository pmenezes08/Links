...
_DEFAULTS: Dict[str, Any] = {
    ...
    # Builder design chat + plan narrations (paid LLM calls that never consume
    # a build turn): free/trial monthly quota; paid tiers uncapped (None).
...
def _load_kb_defaults() -> Dict[str, Any]:
    ...
    out["builder_chat_messages_per_month"] = int(
        _kb_field_value("credits-entitlements", "builder_chat_messages_per_month",
                        _DEFAULTS["builder_chat_messages_per_month"]) or _DEFAULTS["builder_chat_messages_per_month"]
    )
...
def _build_entitlements(
    ...
    ent: Dict[str, Any] = {
        ...
        "builder_chat_messages_per_month": defaults["builder_chat_messages_per_month"],
        # Highest builder quality tier the user may select. Free/trial stop at
        # "balanced"; paid tiers unlock "best" (the premium model). Enforced
        # server-side — the client's requested tier is clamped to this.
        "builder_max_tier": "balanced",
    }
    ...
    if tier == TIER_SPECIAL:
        ent.update({
            ...
            "builder_chat_messages_per_month": None,  # unlimited (paid)
            "builder_max_tier": "best",
