            ...
            "fields": [
                {
                    ...
                    "value": [
                        ...
                        {"title": "Logout session-persistence hotfix (May 2026)", "area": "backend", "phase": "now", "status": "ongoing", "effort": "M", "target_quarter": "2026-Q2", "notes": "Sessions were silently re-authenticating after `/logout` on Capacitor (iOS + Android). Root cause: `rotate_remember_token_after_auto_login` re-issued a fresh `remember_token` on the `/logout` response when remember-me had restored the session in `before_app_request`. Hotfix (PR-A + PR-D + audit log + UI confirm): (a) skip rotation on `/logout` via endpoint check + `g.skip_remember_rotation`; (b) `revoke_for_user` on logout (kills sessions on every device, banking-grade); (c) legacy-domain sweep in `remember_tokens.clear_cookie` mirroring `clear_session_cookie` so pre-May-2026 `.c-point.co` cookies actually expire; (d) audit log on `auto_login_from_remember_token` for post-deploy monitoring; (e) `LogoutPromptProvider` wired in `App.tsx` with explicit multi-device copy. **Deferred follow-ups (P0/P1):** iOS `URLSession.shared` cookie-jar parity (`AppDelegate.syncBadgeWithServer`), Android `CookieManager.flush()`, SW network-only `/api/profile_me`, password-login `ensureAccountIsolation` parity, `delete_account` cookie-clear parity, `logout.test.ts` Vitest hoist fix. Audit + plan: `docs/audit/LOGOUT_REMEDIATION_PLAN.md` + `docs/audit/LOGOUT_SECURITY_REVIEW.md`.", "test": "auth:logout-no-remember-reissue", "test_status": "not_run"},
                    ],
                },
            ],
            "body": (
