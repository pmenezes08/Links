    ...
    return [
        ...
        },
        {
            ...
            "fields": [
                ...
                {"name": "paid_l1_upgrade_cta", "label": "Upgrade CTA shown to owner", "type": "string",
                 ...
                 "group": "paid_l1"},
                {"name": "paid_l1_stripe_price_id_test", "label": "Stripe price ID — monthly (test)", "type": "string", "value": "price_1TrLrUDHGQ57hB63Fhw7TxmJ",
                ...
                {"name": "paid_l2_upgrade_cta", "label": "Upgrade CTA shown to owner", "type": "string",
                 ...
                 "group": "paid_l2"},
                {"name": "paid_l2_stripe_price_id_test", "label": "Stripe price ID — monthly (test)", "type": "string", "value": "price_1TrLrWDHGQ57hB63iZci0Kvh",
                 "help": "Paste the ``price_...`` ID from Stripe test mode. Read by /api/kb/pricing on staging only.",
                 "group": "paid_l2"},
                ...
                {"name": "paid_l3_upgrade_cta", "label": "Upgrade CTA shown to owner", "type": "string",
                 ...
                 "group": "paid_l3"},
                {"name": "paid_l3_stripe_price_id_test", "label": "Stripe price ID — monthly (test)", "type": "string", "value": "price_1TrLrXDHGQ57hB63OiWOsjFq",
                 "help": "Paste the ``price_...`` ID from Stripe test mode. Read by /api/kb/pricing on staging only.",
                 "group": "paid_l3"},
                ...
                {"name": "steve_package_trial_days", "label": "Steve package trial duration (days)", "type": "integer", "value": 14,
                 "help": "Length of the synthetic Steve Community Package trial granted to every "
                         "new root community at creation (no Stripe object; expiry enforced at "
                         "read time). Read by community_billing.grant_steve_package_trial and the "
                         "owner-facing trial countdown.",
                 "group": "trial"},
                ...
                {"name": "community_brain_enabled", "label": "Community Brain enabled", "type": "boolean", "value": True,
                 "help": "Cron-synthesized compact community memory for Steve (steve_community_memory Firestore docs). Turn OFF for emergency disable without redeploy.", "group": "community_brain"},
                {"name": "community_brain_window_days", "label": "Brain activity window (days)", "type": "integer", "value": 30,
                 "help": "Rolling window the synthesis reads; topics/signals outside it fade naturally.", "group": "community_brain"},
                {"name": "community_brain_freshness_days", "label": "Brain freshness limit (days)", "type": "integer", "value": 14,
                 "help": "Steve refuses memory cards older than this — stale memory degrades to no memory, never frozen memory.", "group": "community_brain"},
                {"name": "community_brain_max_communities_per_run", "label": "Brain max communities per cron run", "type": "integer", "value": 10,
                 "help": "Hard cap on metered synthesis calls per cron invocation (one call per community, no retries).", "group": "community_brain"},
                {"name": "community_brain_min_new_posts", "label": "Brain min posts to qualify", "type": "integer", "value": 3,
                 "help": "Root communities need at least this many posts in the window before a synthesis is spent on them.", "group": "community_brain"},
                {"name": "community_brain_max_output_tokens", "label": "Brain synthesis max output tokens", "type": "integer", "value": 600, "group": "community_brain"},
                {"name": "community_brain_card_max_chars", "label": "Brain card max chars in prompt", "type": "integer", "value": 900,
                 "help": "Rendered memory block is capped at this size before injection into Steve replies.", "group": "community_brain"},
                {"name": "feed_thread_summary_enabled", "label": "Long-thread rolling summary enabled", "type": "boolean", "value": True,
                 "help": "On threads longer than the comment window, Steve carries a cached summary of the older comments (one metered call per refresh). Turn OFF for emergency disable.", "group": "paid_steve_package"},
                {"name": "feed_thread_summary_trigger_older", "label": "Summary trigger (older comments)", "type": "integer", "value": 10,
                 "help": "Minimum comments beyond the visible window before a summary is generated.", "group": "paid_steve_package"},
                {"name": "feed_thread_summary_refresh_every", "label": "Summary refresh cadence (new older comments)", "type": "integer", "value": 20,
                 "help": "Cached summary is reused until this many additional comments age out of the window.", "group": "paid_steve_package"},
                {"name": "feed_thread_summary_max_chars", "label": "Summary max chars", "type": "integer", "value": 1500, "group": "paid_steve_package"},
                ...
                {"name": "paid_steve_package_stripe_price_id_test", "label": "Stripe price ID — monthly (test)", "type": "string", "value": "price_1TrLrYDHGQ57hB63RJNoPMyX",
                ...
                {"name": "enterprise_grants_premium_steve", "label": "Members get Premium Steve", "type": "boolean", "value": True,
                 "help": "Default for Enterprise deals: joining opens a Premium-equivalent seat (enterprise_membership.start_seat). A per-deal override on the community row (enterprise_steve_included, set via the admin tier endpoint) wins over this.", "group": "enterprise"},
        ...
        },
        {
            ...
            "fields": [
                ...
                {"name": "weekly_prompts_per_user", "label": "Prompts / user / rolling 7 days (paid)", "type": "integer", "value": 20,
                 "help": "Per-user weekly cap for members of communities with a PAID active Steve Package. Counts user-visible prompts, not internal planner calls."},
                {"name": "networking_requires_steve_package", "label": "Require active Steve Package", "type": "boolean", "value": True,
                 "help": "B2B gate: networking only works for communities whose root has an active Steve Package (paid or 14-day trial). Turn off to make networking available to all members again. App admins, the founder, and Special users are always exempt."},
                {"name": "networking_trial_weekly_prompts", "label": "Prompts / user / rolling 7 days (trial)", "type": "integer", "value": 5,
                 "help": "Reduced per-user weekly cap while a community is on its 14-day Steve Package trial. ~5/week ≈ 10 searches to evaluate networking before buying."},
                {"name": "networking_welcome_cue_enabled", "label": "Rolling-welcome networking cue", "type": "boolean", "value": True,
                 "help": "Append a size-gated 'ask Steve for introductions' link to the weekly rolling-welcome post. Zero-AI attribution experiment; flips off without a deploy."},
                {"name": "networking_welcome_cue_min_members", "label": "Cue minimum community members", "type": "integer", "value": 15,
                 "help": "Rolling-welcome cue only shows in communities with at least this many members — below it everyone already knows everyone and the link is noise."},
                {"name": "planner_model", "label": "Query planner model", "type": "enum",
                 "allowed_values": ["grok-4.3"],
                 "value": "grok-4.3",
                 "help": "Turns a user ask into facets, dimensions, constraints, named people, and a retrieval query."},
                {"name": "planner_reasoning_effort", "label": "Query planner reasoning effort", "type": "enum",
                 "allowed_values": ["none", "low", "medium", "high"],
                 "value": "medium",
                 "help": "How hard grok-4.3 thinks while planning. 'medium' suits intent decomposition; 'none' is the legacy fast/no-reasoning path."},
                ...
                {"name": "large_context_model", "label": "Large-context model", "type": "enum",
                 "allowed_values": ["grok-4.3"],
                 "value": "grok-4.3",
                 "help": "Reserved for very large prompts. grok-4.3 carries a 1M-token context window."},
                ...
                {"name": "fallback_model", "label": "Fallback model", "type": "enum",
                 "allowed_values": ["grok-4.3", "grok-4.20-multi-agent"],
                 "value": "grok-4.3"},
                {"name": "planner_input_usd_per_million", "label": "Planner input $ / 1M tokens", "type": "decimal", "prefix": "$", "value": 1.25},
                {"name": "planner_output_usd_per_million", "label": "Planner output $ / 1M tokens", "type": "decimal", "prefix": "$", "value": 2.50},
            ...
            "body": (
                ...
                "- **Query planner**: converts a user prompt into facets, constraints, dimensions, named people, and search rewrite. Default: `grok-4.3` at `medium` reasoning effort.\n"
                "- **Final answer**: writes the recommendation from the bounded roster. Default: `grok-4.3`.\n"
                "- **Large-context fallback**: reserved for very large prompts. Default: `grok-4.3` (1M-token context window).\n"
        ...
        # ── Pricing ─────────────────────────────────────────────────
        {
            ...
            "fields": [
                ...
                {"name": "model_costs_last_checked", "label": "Model costs last checked", "type": "date", "value": "2026-06-29", "group": "model_costs"},
        ...
        # ── Policy ──────────────────────────────────────────────────
        {
            ...
            "field_groups": [
                ...
                {"id": "chat_memory", "label": "Steve chat memory (Phase 3)", "icon": "fa-brain"},
            ],
            "fields": [
                # Per-turn
                ...
                {"name": "max_context_messages_peer_dm", "label": "Max context messages — peer DM (@Steve in a human↔human DM)", "type": "integer", "value": 60,
                ...
                {"name": "thread_summary_trigger_messages", "label": "Thread summary trigger (message count beyond verbatim window)", "type": "integer", "value": 120,
                 "help": "Only generate a summary when older-than-window messages exceed this threshold.", "group": "per_turn"},
                {"name": "thread_summary_refresh_messages", "label": "Thread summary refresh cadence (new messages since last summary)", "type": "integer", "value": 40,
                 "help": "Re-summarize when this many new messages have arrived since the last summary.", "group": "per_turn"},
                ...
                {"name": "chat_memory_chunk_messages", "label": "Messages per memory chunk", "type": "integer", "value": 60,
                 "help": "Target number of chat messages per text chunk before embeddings are added.", "group": "chat_memory"},
                {"name": "chat_memory_chunk_chars", "label": "Max characters per memory chunk", "type": "integer", "value": 3200,
                 "help": "Hard character budget for a single stored chat-memory chunk.", "group": "chat_memory"},
                {"name": "chat_memory_top_k", "label": "Older-memory chunks injected", "type": "integer", "value": 4,
                 "help": "Maximum retrieved older-memory chunks to format into Steve's prompt once retrieval is enabled.", "group": "chat_memory"},
                {"name": "chat_memory_max_prompt_chars", "label": "Older-memory prompt character budget", "type": "integer", "value": 3500,
            ...
            "fields": [
                {
                    ...
                    "value": [
                        ...
                        {"title": "Native page transitions — pilot (Dashboard / Feed / Community / Post)", "area": "client", "phase": "now", "status": "ongoing", "effort": "L", "target_quarter": "2026-Q3", "notes": "Frame-0 instant paint + push/pop (250ms) + tab cross-fade (120ms) on pilot routes. Feature flag `VITE_PAGE_TRANSITIONS`. Recursive QA loops until PO sign-off. Chat transitions deferred. Tracking: `docs/workflow-state.md`.", "test": "manual:page_transitions_pilot", "test_status": "not_run"},
                        {"title": "Pilot Wave 2 — composer & profile polish", "area": "client", "phase": "now", "status": "ongoing", "effort": "L", "target_quarter": "2026-Q3", "notes": "Wave-2 follow-on to the Wave-1 page-transitions pilot. 7 PRs: (1) GIF picker rebuild as keyboard-aware bottom-sheet with liquid-glass tokens and denser grid; (2) Profile + PublicProfile frame-0 skeletons; (3) AccountSettings frame-0 skeleton; (4) Notifications + Followers frame-0 skeletons; (5) Capacitor Haptics util + opt-in callsites (send / pop-back / GIF select / long-press); (6) multi-step pop-back animation Post → Community → Tab; (7) Pull-to-refresh visual on Feed using pilot tokens. Reuses `VITE_PAGE_TRANSITIONS` flag (staging-only). **Deferred / out of scope:** motion or layout changes to `ChatThread.tsx`, `GroupChatThread.tsx`, `Messages.tsx`, `Communities.tsx` — those need their own design pass after the Hot-traffic Wave A/B revert. Tracking: `docs/workflow-state-wave-2.md`.", "test": "manual:pilot_wave_2", "test_status": "not_run"},
                        {"title": "Capacitor StatusBar for consistent native UI", "area": "client", "phase": "now", "status": "shipped", "effort": "S", "target_quarter": "2026-Q3", "notes": "Dynamic status bar style in DM/group/chat media pages; immersive hide in ChatMediaViewerModal and ChatMedia fullscreen. Requires cap sync + native rebuild (@capacitor/status-bar). Pairs with iOS chat scroll open-at-bottom fix.", "test": "native:statusbar-ui", "test_status": "not_run"},
                        ...
                        {"title": "Conversion + retention MVP — trial value, member digest, funnel attribution", "area": "Subscriptions", "phase": "now", "status": "ongoing", "effort": "L", "target_quarter": "2026-Q3", "notes": "July 2026: (1) Owner Dashboard `steve_trial` metric (KB-driven trial days via `community-tiers.steve_package_trial_days`, shared-pool usage, `action_trial_ending` → ManageMembershipModal); (2) member weekly digest cron `/api/cron/member-weekly-digest` (recipient-locale, `member_digest_sends` dedup, OFF by default via `MEMBER_DIGEST_ENABLED`); (3) retention attribution sink `retention_events` + `POST /api/retention/event` (digest/pulse tap-through, owner action taps) and `<plan>_checkout_started` rows in `subscription_audit_log` with closed CTA source vocabulary; (4) KB-derived landing pricing artifact (`scripts/generate_landing_pricing.py` → `landing/src/generated/pricing.json`, drift-checked in CI). Docs: `docs/PRODUCT_JOURNEYS.md`, `docs/cloud-scheduler-cron.md` §13, `docs/QA_CHECKLIST.md` §18.", "test": "pytest:member_digest+retention_events+steve_trial_value", "test_status": "not_run"},
                    ],
                },
            ],
            "body": (
            ...
            "fields": [
                {
                    ...
                    "value": [
                        ...
                        {
                            "id": "steve_chat_memory:skeleton",
                            "feature": "Steve Phase 3 Chat Memory",
                            "behaviour": "Config parsing, exact DM/group scope helpers, Firestore sidecar paths, reset/privacy filters, and prompt-section formatting stay deterministic and vendor-free.",
                            "runner": "pytest",
                            "target": "tests/test_steve_chat_memory.py",
                            "status": "not_run",
                            "last_run_at": "", "last_run_by": "", "last_run_notes": "",
                        },
                        {
                            "id": "manual:steve_chat_memory_flags",
                            "feature": "Steve Phase 3 Chat Memory",
                            "behaviour": "Chat-memory KB flags default off, entitlement projection matches KB edits, and disabled rollout creates no Firestore memory or live prompt changes.",
                            "runner": "manual",
                        ...
                        {
                            "id": "manual:pilot_wave_2",
                            "feature": "Pilot Wave 2 — composer & profile polish",
                            "behaviour": "PR-1 GIF picker rebuild — see §15.A. GIF picker bottom-sheet (keyboard-aware, glass, denser grid, drag-to-dismiss, right-aligned attribution); Profile/PublicProfile/AccountSettings/Notifications/Followers frame-0 skeletons; Capacitor Haptics on send/pop-back/GIF select/long-press; multi-step pop-back Post→Community→Tab; Feed PTR visual using pilot tokens. Reuses `VITE_PAGE_TRANSITIONS` (staging only). Chat/Messages/Communities motion explicitly out of scope. Tracking: `docs/workflow-state-wave-2.md`.",
                            "runner": "manual",
                            "target": "QA_CHECKLIST.md §15",
                            "status": "not_run",
                            "last_run_at": "", "last_run_by": "", "last_run_notes": "",
