        ...
        },
        {
            ...
            "fields": [
                ...
                {"name": "paid_l1_stripe_price_id_test", "label": "Stripe price ID — monthly (test)", "type": "string", "value": "", "tbd": True,
                 ...
                 "group": "paid_l1"},
                {"name": "paid_l1_stripe_price_id_live", "label": "Stripe price ID — monthly (live)", "type": "string", "value": "price_1TYTjADHGQ57hB63rdjUVz0m",
                ...
                {"name": "paid_l2_stripe_price_id_test", "label": "Stripe price ID — monthly (test)", "type": "string", "value": "", "tbd": True,
                 "help": "Paste the ``price_...`` ID from Stripe test mode. Read by /api/kb/pricing on staging only.",
                 "group": "paid_l2"},
                {"name": "paid_l2_stripe_price_id_live", "label": "Stripe price ID — monthly (live)", "type": "string", "value": "price_1TYTjFDHGQ57hB63W4Zai1uE",
                 "help": "Paste the ``price_...`` ID from Stripe live mode. Read by /api/kb/pricing on production only.",
                 "group": "paid_l2"},
                ...
                {"name": "paid_l3_stripe_price_id_test", "label": "Stripe price ID — monthly (test)", "type": "string", "value": "", "tbd": True,
                 "help": "Paste the ``price_...`` ID from Stripe test mode. Read by /api/kb/pricing on staging only.",
                 "group": "paid_l3"},
                {"name": "paid_l3_stripe_price_id_live", "label": "Stripe price ID — monthly (live)", "type": "string", "value": "price_1TYTjODHGQ57hB63ZYrKcQHR",
                 "help": "Paste the ``price_...`` ID from Stripe live mode. Read by /api/kb/pricing on production only.",
                 "group": "paid_l3"},
            ...
            "fields": [
                {
                    ...
                    "value": [
                        ...
                        {"title": "Usage / credit calculator (admin)", "area": "admin", "phase": "now", "status": "ongoing", "effort": "M", "target_quarter": "2026-Q2", "notes": "Expand KB **credits-entitlements** / admin calculator for operator-grade **token math**: separate **input vs output** $/M (and cached input when relevant), add **tool-call line items** (`tool_call_per_1000_usd`), **USD→EUR** via `usd_to_eur_rate`, and **scenario presets** (e.g. minimal onboarding, +`onboarding_parse_cv`, +`onboarding_compose_bio`, +`onboarding_company_intel` / web-search-heavy path) with illustrative **p50/p95** token assumptions. Document how **`ai_usage_log`** fields (`tokens_in`, `tokens_out`, `cost_usd`, `model`, `request_type`, `surface`) map to estimates; stress that **`monthly_spend_ceiling_eur`** only matches reality when **`cost_usd`** is populated on gated paths. Pairs with **Steve call credits** row."},
                        ...
                        {"title": "Steve call credits — weighted debits and entitlement parity", "area": "backend", "phase": "now", "status": "not_started", "effort": "M", "target_quarter": "2026-Q2", "notes": "Persist **integer credit weight** per successful Steve (and heavy non-Steve if needed) row on `ai_usage_log`; change `monthly_steve_count` / `daily_count` (and community pool counters if applicable) to **SUM(COALESCE(weight,1))** vs `steve_uses_per_month`. Encode multipliers in KB (DM=1, group/feed=3, post summary=2, web-search / company-intel=N) per **credits-entitlements** internal weights. Ensure **`check_steve_access`** and **`monthly_spend_ceiling_eur`** (SUM `cost_usd`) cannot block a user while the UI still shows remaining **call** credits—either **effective_remaining = min(credits_left, spend_budget_left)** in API or tune caps so ceiling binds only as safety net. Blocks: `entitlements_gate`, `ai_usage.py`, all Steve `log_usage` call sites.", "test": "entitlements:weighted-credits", "test_status": "not_run"},
                        {"title": "Credit tracking + display (X / 100 Steve uses)", "area": "client", "phase": "now", "status": "not_started", "effort": "M", "target_quarter": "2026-Q2", "notes": "Client/UX for monthly/daily Steve allowance: read **`monthly_steve_used` / cap** from API once backend uses **weighted** `SUM` (see **Steve call credits** row). Show **consistent remaining** with server enforcement (no credits left while blocked for spend ceiling). Reuse `ManageMembershipModal` / `useEntitlements` patterns per AGENTS.md."},
            ...
            "fields": [
                {
                    ...
                    "value": [
                        ...
                        {
                            "id": "entitlements:weighted-credits",
                            "feature": "AI usage — Steve credits",
                            "behaviour": "monthly_steve_count / daily_count use SUM(credit weight) on ai_usage_log; UI remaining matches gate; spend ceiling vs credits parity.",
                            "runner": "pytest",
                            "target": "tests/test_ai_usage_counters.py (TBD: add weighted-credits cases)",
                            "status": "not_run",
                            "last_run_at": "", "last_run_by": "", "last_run_notes": "",
                        },
                        {
