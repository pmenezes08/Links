...
def test_select_image_urls_only_when_user_asks():
    ...
    assert select_image_urls_for_turn(urls, "what is in this photo?", max_count=5) == urls
    assert select_image_urls_for_turn(urls, "hello", max_count=5) == []
    assert select_image_urls_for_turn(urls, STEVE_SHARED_PHOTO_USER_MESSAGE, force=True, max_count=1) == [
