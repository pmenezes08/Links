...
| **Steve's build capabilities** change (new `CPoint` API / route / connector, new build kind, design-direction change, new build quality pass) | Edit **`backend/services/builder_guide.md`** (the guide injected into every build; its CAPS block is shared into Steve's chat prompt, so this keeps builds AND chat feedback accurate). |
