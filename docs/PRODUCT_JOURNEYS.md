...
- Steve document retrieval is two-stage. `/upload_doc` writes the canonical MySQL `useful_docs` row and file bytes to R2/local storage first, then queues best-effort **`backend.services.steve_document_memory`** indexing into Firestore (`steve_doc_memory/{community:id|group:id}/docs/{doc_id}/chunks/{chunk_id}`). Feed/group Steve turns assemble calendar/links/docs/polls through **`backend.services.steve_resource_context`**, which loads a compact exact-scope document manifest and retrieves only relevant PDF chunks when the current thread, parent/original post, recent replies, or recent upload state makes a document ask active (`should_include_community_resources_from_thread` with `has_recent_docs=True`). Large PDFs are never injected wholesale; group docs use `group:{id}` memory only and are not visible from the parent community feed. Legacy on-the-fly PDF text extraction only fires when memory has not indexed anything for that scope. Existing `useful_docs` rows can be backfilled with **`scripts/backfill_steve_document_memory.py`**.
...
**DMs / group chat:** Canonical thread metadata, rules, and `messages` table in **MySQL** (with `image_path`, `video_path`, `media_paths` JSON); **Firestore** (`dm_conversations/{conv_id}/messages`) is primary for realtime/paginated reads when a conversation document exists (see **`MYSQL_AND_FIRESTORE.md`**, `dm_chats` / `group_chat` routes in **`BACKEND_ROUTES.md`**). On **initial full load**, if Firestore has no conversation or returns an empty page, `/get_messages` **falls back to MySQL** so thread-list rows (MySQL-sourced) still open with history. Delta (`since_id`) and backward (`before_id`) Firestore reads may legitimately return empty without falling back. Dual-write is best-effort. Link previews integrate via `LinkPreview.tsx` (used in `MessageBubble.tsx` etc.); see below for full media flow.
