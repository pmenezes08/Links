...
**Recent changes**: Direct R2 client upload for >25MB videos, `media_paths` for grouped media (now fully persisted), iOS MIME, CF optimizer + ACL/crossOrigin. **This change**: iOS preview broken image icon fixed (absolute error overlay z-10 in MessageImage, errorCount in key/useMemo for remount, wrapped raw grid img in ChatMedia). False "You're offline" banner on cold online start fixed (isInitialized state + symmetric getStatus/goOnline in NetworkContext; updated OfflineBanner condition). Updated living docs, KB roadmap. New iOS build required (npx cap sync ios + Xcode/TestFlight for plugins).
