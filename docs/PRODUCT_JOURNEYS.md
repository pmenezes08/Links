...
- Community-feed Steve uses the same community-aware runtime whether the payer is the community add-on or a Premium member's personal allowance: Grok 4.3, no multi-agent, compact Firestore community memory, selective MySQL context, and **hosted `web_search` / `x_search`** per **`steve_tool_policy`** when intent qualifies: phrase-based live-info signals, optional KB default-on web/X (when explicit-only is off), and KB **`paid_steve_package_feed_attach_*`** toggles that enable/disable each hosted tool without deploy. Logs record the effective tool list. The gate decides attribution before vendor spend: active add-on uses `community_id=<root_id>`; Premium personal fallback logs with `community_id=NULL`.
