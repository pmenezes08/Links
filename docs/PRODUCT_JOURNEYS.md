...
- Steve document retrieval is two-stage. `/upload_doc` writes the canonical MySQL `useful_docs` row and file bytes to R2/local storage first, then queues best-effort **`steve_document_memory`** indexing into Firestore. Feed/group Steve turns load a compact exact-scope document manifest and retrieve only relevant PDF chunks when the current thread, parent/original post, recent replies, or recent upload state makes a document ask active. Large PDFs are never injected wholesale; group docs use `group:{id}` memory only and are not visible from the parent community feed.
