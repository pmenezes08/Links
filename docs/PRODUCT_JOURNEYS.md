...
**Recent changes**: Direct R2 client upload for >25MB videos, `media_paths` for grouped media (now fully persisted), iOS MIME, CF optimizer + ACL/crossOrigin. **This change**: App loads without server data on new Xcode build fixed (NetworkContext initial state always navigator.onLine to prevent cached/offline mode; profile defaults in firestore_reads.py:350 to avoid ghost account; plist FirebaseAppDelegateProxyEnabled=false for PushNotifications event; welcome images bundled). No banner but seems offline due to initial isOnline. Updated AppDelegate serverURL, dashboardCache clear, docs. New Xcode build required (clean, cap sync, entitlements for app.c-point.co).
