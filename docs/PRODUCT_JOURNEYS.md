...
- Community-feed Steve uses the same community-aware runtime whether the payer is the community add-on or a Premium member's personal allowance: Grok 4.3, no multi-agent, compact Firestore community memory, selective MySQL context, and **hosted `web_search` + `x_search`** only when **`steve_tool_policy`** detects live/current-info intent (expanded phrasing includes e.g. *today's news*, *headlines*, *latest news*, *happening today*) or when KB enables default web/X for the Steve Community Package. Logs record the effective tool list. The gate decides attribution before vendor spend: active add-on uses `community_id=<root_id>`; Premium personal fallback logs with `community_id=NULL`.
...
**Recent changes**: Direct R2 client upload for >25MB videos, `media_paths` for grouped media (now fully persisted), iOS MIME, CF optimizer + ACL/crossOrigin. **This change**: Preview "Unavailable" fixed with safe retry to original src in MessageImage (for thumbnails in bubble/list/ChatMedia). Added Capacitor Network (offline detection) + Filesystem (local save to Documents for phone memory/native feel, manual button in viewer) — highest ROI/safest (opt-in, permission guard, fallback to IndexedDB). Updated capacitor.config.ts, knowledge_base.py roadmap_items (added Haptics/StatusBar/App/Storage with descriptions/purposes for native UX/offline), created backup branch. Updated living docs. New iOS build required (npx cap sync ios + Xcode/TestFlight for plugins).
