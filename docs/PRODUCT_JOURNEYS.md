...
- **SSO linking is safe-by-default.** Both `/api/auth/google` and `/api/auth/apple` capture the currently-authenticated `session["username"]` (validated against `users` in the same DB cursor) before mutating state. When a logged-in user taps Continue with Apple/Google and the SSO identity isn't yet linked to any C-Point account (common path on iOS when Apple returns a private relay email), the route **links the SSO id to the active session user** instead of silently creating a duplicate row that has no community memberships (the symptom: Links & Docs returning 403 on iOS for affected accounts). When the SSO id (or signed email) resolves to a **different** C-Point user than the active session, the route returns `409 sso_belongs_to_other_user` / `sso_email_belongs_to_other_user` with `username_on_token` / `username_on_email` so the UI can surface "sign out first". Callers that know the user already has an account can pass `link_only: true` to refuse new-user creation entirely (`404 no_matching_account` instead of spawning a row). `client/src/pages/MobileLogin.tsx` maps these codes to readable messages via `ssoErrorMessage`.
- **Session finish is shared.** Successful OAuth and password paths clear stale session state, issue the normal remember-token/install cookies, and invalidate profile/dashboard caches. The client **SPA-navigates** to `/premium_dashboard` (no full reload) and then runs push-token re-registration, dashboard server-pull, and profile refresh in the background so first paint isn't blocked on a waterfall. The shared `before_request` email-verification gate caches `email_verified` in the Flask session after the first positive lookup, which keeps subsequent authenticated API calls from re-running the same `SELECT` per request. `remember_tokens.ensure_tables` caches its DDL after first success so login / restore / revoke don't pay an idempotent schema roundtrip on every request.
...
- Community-feed Steve uses the same community-aware runtime whether the payer is the community add-on or a Premium member's personal allowance: Grok 4.3, no multi-agent, exact-scope Firestore/MySQL corpus via **`backend.services.steve_community_context`** (current community/group only: posts, comments/replies, authorized media, links, document excerpts, calendar events, tasks, polls), and **hosted `web_search` / `x_search`** when **`steve_tool_router.resolve_steve_hosted_tools`** attaches them: first **`steve_tool_policy.steve_tools_for_message`** (explicit browse phrasing, **news/sports/markets** heuristics from **`steve_prompt_policy`**, job/careers-site wording, or KB “default on” when **external search explicit only** is OFF); if that yields no tools, an optional **JSON-only tool-router** pass may attach hosted tools on **ambiguous public web / X** intent (skipped for platform-manual-only and professional-advice-only turns, profile-suppressed cohorts, `STEVE_TOOL_ROUTER_DISABLED`, logged as `request_type=steve_tool_router`). Profile / “who is @user” style asks stay **platform-first** (no external tools) unless the same message also requests live news. KB **`paid_steve_package_feed_attach_*`** can still disable either channel; **DM and group @Steve** read the same policy from **`SteveCommunityConfig`**. Platform-manual and professional-advice-only turns never attach tools. Logs record the effective tool list. The gate decides attribution before vendor spend: active add-on uses `community_id=<root_id>` for billing attribution; Premium personal fallback logs with `community_id=NULL`. **News-style** asks map to **`steve_prompt_policy` `news_current_events`** (structured briefing + headline Markdown sources + tier hygiene); **`steve-platform-manual` / `steve.what_can_i_do`** documents Steve capability boundaries for product questions. **@Mentioned** members (other than the asker) may receive gated Firestore profile blocks on feed/group replies per **`user_can_access_steve_kb`**, but that profile gate never widens community-resource reads.
- Steve document retrieval is two-stage. `/upload_doc` writes the canonical MySQL `useful_docs` row and file bytes to R2/local storage first, then queues best-effort **`steve_document_memory`** indexing into Firestore. Feed/group Steve turns only enter document memory when the caller asks for resources or the current thread/original post/recent replies contain document intent; then they load exact-scope document memory and retrieve only relevant PDF chunks. Large PDFs are never injected wholesale; group docs use `group:{id}` memory only and are not visible from the parent community feed.
...
**Post detail loading.** `client/src/pages/PostDetail.tsx` no longer dual-fetches `/api/group_post` and `/get_post` in parallel. It uses the navigation state hint (`location.state.groupId`, set by `GroupFeed.tsx`) or the cached `isGroupPost` flag to pick the right endpoint first, falling back to the other only on 404. Backend reply hydration for `/get_post` (MySQL fallback), `/api/group_post`, and `/api/reply/<reply_id>` is now batched with `IN (...)` / `GROUP BY` queries for profile pictures, reactions, user reactions, view counts, and child counts — replacing the per-reply queries that were timing out on heavy threads. `/api/group_post` no longer runs `ALTER TABLE group_replies ADD COLUMN audio_path|audio_summary` on the read hot path; those columns are ensured in the consolidated startup migration block alongside `useful_links/useful_docs.group_id`.
**Links & Docs access** matches feed visibility. `_can_view_community_content` in `bodybuilding_app.py` covers app admin, community creator, direct members, **and** admins/owners of any ancestor community via `get_parent_chain_ids` + `is_community_admin`, so sub-community Links & Docs stay visible to parent-network admins. `backend/services/useful_links_read.py` no longer silently drops the docs list when a deployment is missing `useful_docs.details` or `useful_docs.group_id`; missing optional columns now log a warning and degrade to the community-only filter. The Links & Docs page surfaces backend errors (`load_failed`, `access_denied`) instead of rendering a blank list.

**Links & Docs on iOS.** Two iOS-only fixes shipped to bring parity with web:

1. The **Open** button on each link/doc card now routes through `openExternalInApp` (`client/src/utils/openExternalInApp.tsx`). iOS WKWebView silently swallows `<a target="_blank">` taps, which used to make Open do nothing on iOS for R2-hosted PDFs and external links. The helper opens the URL via `@capgo/inappbrowser` on native (with `window.open` fallback) and an in-app overlay on web. `resolveDocUrl` is wrapped with `absoluteDocUrl` so relative `/uploads/...` paths get the WebView origin prefix before being handed to the plugin.
2. `UsefulLinks.load()` now detects `@login_required`'s `302 → /login → 200 (HTML)` bounce (`r.redirected` / `r.url.pathname === '/login'` with a non-JSON content-type) and surfaces `links_docs.access_denied` instead of the generic `links_docs.load_failed`. This was the silent-failure mode when WKWebView dropped cookies after an app upgrade or ITP wipe.
