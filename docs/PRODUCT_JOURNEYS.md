...
**Membership implies the ancestor chain (July 2026).** Adding a user to a nested community also adds them to its owning sub-community and the root network: `community.ensure_ancestor_memberships` runs inside the monolith's `add_user_to_community` chokepoint (invites, join requests, admin adds), root-first and BEFORE the target insert, so a capped Free/Paid root aborts the whole join with the normal `CommunityMembershipLimitError`; ancestors already joined are skipped. The Enterprise seat hook resolves the **root** tier (`resolve_root_community_id`) so an invite pointed at a nested node of an Enterprise-with-Steve network still opens the seat. Tests: `tests/test_ancestor_membership.py`.

**Guided placement (July 2026).** Enterprise roots can auto-allocate accepted members into private sub-communities via an owner-authored questionnaire, delivered by Steve but fully deterministic — every option maps to `target_community_ids` and allocation is the union of the picked options' targets, so there is **no AI call and no `ai_usage_log` row**. Owner config: `GET/POST /api/community/<id>/placement/config` (manage-gated, root-only, Enterprise-only, targets validated as descendants; option labels are the only text members ever see pre-allocation). Flow: the accept paths (invite accept, token accept, join-request approval) call `community_placement.open_pending_placement_if_active` inside the accept transaction; the client discovers pendings via `GET /api/me/placement/pending` on app open (Steve welcome uses the stored `inviter_username`), and `POST /api/community/<id>/placement/respond` validates mandatory answers (single-choice = exactly one; multi-choice may be empty), inserts memberships through `add_user_to_community`, and completes the row. The questionnaire is persistent, not blocking: "I'll do this later" routes through a Steve confirmation ("sub-communities are waiting for you") and then snoozes for the current app session only (sessionStorage; the server row stays pending), the modal returns on next app open, and a pinned Steve card in the root community feed (`PlacementPendingCard`, reopening via the `cpoint:placement-open` event) keeps re-entry visible until answered. The gate re-checks tier + KB `enterprise_guided_placement_included` + active questions at every read, so downgrades or disabling dissolve pendings silently. Tests: `tests/test_community_placement.py`.
...
- **Silent decline, open door.** Declines never notify and no reason is ever recorded or shown; request-pending copy promises only the positive path ("If you're welcomed in, you'll hear here"). But a decline does not lock the requester out: their state resets immediately (lookup returns `request_status: null`), so they can ask again right away and the owner/admins receive a fresh push + notification — an owner who rejected by mistake just accepts the second knock. Repeat knocks are bounded by the requester's daily join-request rate limit.
