...
5. **Gate before you spend.** Run the entitlements gate *before* calling
   ...
   **Image format constraint (all surfaces):** xAI vision downloads each attached URL and **400s the entire request** if any resolves to a content-type outside jpeg/png/webp/ico — gif, svg, and extension-less R2 keys (served as `application/octet-stream`) are rejected. **`steve_chat_images.filter_xai_supported_image_urls`** must gate every attachment (it is built into `select_image_urls_for_turn` / `build_grok_user_content`; the monolith feed/group-post paths call it inline), and Grok calls that attach images go through **`create_response_with_image_fallback`**, which retries text-only when xAI rejects an image so Steve never goes silent over a bad attachment.
...
| `SURFACE_BUILDER_LLM` | Per-call metering rows for the raw LLM calls inside one build job (codegen, research, repair/refine regens), written context-gated by `llm.usage_context(surface='builder_llm')` in `run_build_job`. Carries real `tokens_in/out` + `cost_usd` for the spend ceilings; deliberately distinct from `SURFACE_BUILDER` so a multi-call build still consumes exactly one turn. Not in `STEVE_SURFACES`. |
| `SURFACE_BUILDER_CHAT` | Steve's pre-build design conversation (`builder.converse`). Distinct so chatting never consumes a build turn, but gated by its own monthly allowance (`builder_chat_messages_per_month`, shared with `builder_plan`; `gate_builder_chat_or_reason`) and per-user rate limits. Rows are metered (real model/tokens/cost) via `usage_context`. Not in `STEVE_SURFACES`. |
| `SURFACE_BUILDER_PLAN` | The tiny "here's what I'll make" narration call while a build runs. Shares the `builder_chat_messages_per_month` allowance; never counts against the build cap. Metered via `usage_context`. Not in `STEVE_SURFACES`. |
| `SURFACE_BUILDER_JUDGE` | The vision-judge grading a rendered build (drives render-fix / data verification / design-refine repairs). One row per judge pass — a successful pass logs a metered row (real tokens/cost) from the nested `usage_context` in `vision_judge.judge`; a failed pass logs one `success=0` row. A single call may carry 2–3 labeled screenshots (mobile fold, full page, desktop) for websites/apps. Never counts against the build cap. Not in `STEVE_SURFACES`. |

**Builder model routing** is three-tier (`BUILDER_TIERS` in `builder.py`: fast=Grok 4.3, balanced=Grok 4.3 + design-refine (grok-4.5 planned — blocked on xAI EU region availability, see `builder.py`), best=Claude Fable 5 (server-side refusal fallback to Opus 4.8); env-overridable, users only ever see the Quick/Polished/Showpiece labels) and every tier still routes through `content_generation/llm.py` — never a vendor SDK directly. The build pipeline, verification pass, and artifact-hygiene guards are documented in `docs/STEVE_BUILD.md`; the injected build guide is `backend/services/builder_guide.md` (living doc).
