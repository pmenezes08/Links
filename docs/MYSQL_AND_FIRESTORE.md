...
| `posts`, `post_views` | `init_database.py`, `post_views.py`, `backend/services/steve_community_welcome.py` | Feed posts + view counts. Steve-authored cold-start posts use `is_system_post=1`, `author_kind='system'`, and `welcome_card_key` values such as `welcome.root`, `cold_start.poll.v1`, `cold_start.introduce_yourself.v1`, and `cold_start.rolling_welcome.v1`; Firestore `posts/{post_id}` mirroring is best-effort after MySQL commits. |
