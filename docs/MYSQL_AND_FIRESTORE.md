...
| `communities` | `init_database.py`, monolith, `backend/services/steve_community_welcome.py` | Community rows, hierarchy. Cold-start bookkeeping columns are added idempotently by Steve welcome helpers: `welcome_post_id`, `cold_start_poll_post_id`, and `introduce_thread_post_id`. `recommended_profile_mode` (`none`, `personal`, `professional`, `both`) is an owner-controlled soft recommendation surfaced in the feed; it is not an authorization gate. |
