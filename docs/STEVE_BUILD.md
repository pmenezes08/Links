...
- `POST /api/cron/builder/play-digest` - weekly creator digest ("N opens this week"), zero-LLM, snapshot-idempotent, `X-Cron-Secret`-gated.
...
- **Sections and sub-categories.** Every creation has a section (`_public_kind`: website/app/game) and an optional `category` slug inferred at build time by the free keyword classifier `infer_creation_category` (no model call) within its resolved section — a "travel game" can never land in app/travel. The closed taxonomy lives in `builder.BUILDER_CATEGORIES` (a display vocabulary constant, deliberately NOT in the Knowledge Base, which is scoped to pricing/caps/policy) and is served to clients in the explore payload's `taxonomy` block and via `GET /api/builder/taxonomy`. `category` may be NULL (untagged); untagged creations always still list under their section — there is no "Other" bucket and nothing is ever hidden for lacking a tag.
- **Category precedence (founder-ratified 2026-07-12): admin (locks) > creator > llm > keyword**, recorded in `creations.category_source`. Creators set/clear their build's category from the My Builds action sheet (`POST /api/builder/<id>/category` — owner-only, non-enumerating, slug validated against the creation's section server-side); automation (`infer_creation_category`, `ensure_gallery_meta`) never overwrites a human source; an explicit admin pick on approve stamps `source='admin'` and locks (creator writes then return `409 category_locked`). A no-category admin approve never touches an existing assignment.
- **Steve's in-chat shelf capture.** `converse` emits a `category` sidecar field on its existing metered chat turn (zero extra spend): when proposing a NEW build, Steve names the shelf in his reply ("it'll sit under Apps › Travel — easy to change later") and the slug flows client → `POST /api/builder/create` → `builder_jobs.suggested_category` → `create_creation`, where it is re-validated against the FINAL kind (mismatch falls back to the keyword classifier; applied slugs stamp `source='llm'`). The sidecar never influences `ready`/`brief`; out-of-enum slugs are dropped at parse time. The taxonomy lives in the chat prompt (`_CONVERSE_CATEGORY`), deliberately NOT in `builder_guide.md` (codegen-only, token-sensitive).
- **Fine-grained browse.** Shelf chips filter in place; "See all" carries the active chip into a deep-linked `?kind=&category=` grid. Deep grids with ≥ 12 items gain a client-side search field over `title + gallery_hook` only (never `prompt_history` — anonymity); a search matching 0-1 items shows a "No X here yet — build the first?" nudge seeding `/builder` instead of a lonely grid (1 is deliberately treated as 0). Graduation ladder if search demand outgrows the client window: server `?q=`, then curated taxonomy depth — never speculatively.
...
- The client renders App-Store-style horizontal shelves when supply is rich (every section ≥ 3 items and ≥ 12 total) and collapses to a single mixed grid below that, so thin supply never reads as empty shelves. Sub-category chips are supply-gated per shelf (≥ 2 distinct categories AND ≥ 6 items; only non-empty chips render) and filter the shelf in place. Play counts render only at ≥ 10 ("social-proof floor"). Cards carry a ghost **"Build your own"** hook that hands `/builder?remix=<id>&seed=<localized prompt>` to the builder — the seed is prefilled and editable (never auto-sent), and the first build of that session goes through the remix route below.
- **Curation pass (classify + hook).** On any transition to `gallery_status='approved'`, `builder_gallery_meta.ensure_gallery_meta` makes ONE cheap metered LLM call (surface `content_gen`, request_type `builder_gallery_meta`, real tokens/cost via the shared `usage_context`) that fills the missing sub-category (closed per-section enum) and writes `gallery_hook` — a one-line Steve-voiced card hook shown instead of the boilerplate. Idempotent (no spend when both fields exist), best-effort (listing never fails on it), never overwrites an existing/admin-set category, event-driven only — deliberately NOT a cron. Backfill for pre-existing approved rows: `scripts/backfill_gallery_categories.py` (free keyword pass by default; `--hooks` adds the metered pass).
- **Creator return loop.** `POST /api/cron/builder/play-digest` (weekly) notifies creators whose gallery-listed creations grew by ≥ 3 plays since the last digest ("{title} was opened {count} times this week", localized via `notification_copy` in the recipient's locale, in-app + push, linking to My Builds). Zero LLM. Idempotent via the `plays_digest_count`/`plays_digest_at` snapshot, which only advances after a successful send.
...
- **Covers.** On approval, `capture_gallery_cover` renders the **sanitized render-stub artifact** (the exact document the vision judge screenshots — all CPoint session/community APIs stubbed, so no identity or viewer data can reach the pixels) and persists one poster PNG to private R2 (`gallery/covers/<id>.png`, `gallery_cover_key`). Served only for approved creations via `GET /api/builder/explore/<id>/cover` with edge caching. Entirely best-effort: without the render service or R2 the client keeps its deterministic gradient covers, which also remain the fallback for cards past the live-preview budget. A persisted cover replaces the card's live iframe preview (plain cacheable `<img>`, no CPU).
- **Featured.** `gallery_featured` is a **human-curated** admin pick (`action: feature|unfeature` on the admin gallery route only — the service enforces `is_admin`; deliberately never an LLM cron per the July credit-burn lesson). Featured rows sort first in the payload and the client renders a large-card Featured shelf above the catalog.
- **Builder pseudonyms (opt-in gallery credit).** `builder_pseudonyms` maps a username to ONE self-chosen Explore handle. Privacy rules: never resolves to a username/avatar/profile (no link, no lookup); may not match ANY existing username (case-insensitive) and must be unique across builders — both rejected with the same non-enumerating `pseudonym_unavailable`; clearing it returns all the member's listings to fully anonymous. The explore payload's `builder` field carries the pseudonym (or null), enabling the card credit ("by …") and the `?builder=` "more from this builder" filter. Set from the My Builds action sheet once a creation is listed. This feature deliberately touches no Steve KB/profiling path (see `docs/STEVE_PRIVACY_GATE.md` — the KB gate is unrelated and unchanged).

### Remix ("Build your own")

`POST /api/builder/<id>/remix` builds a NEW creation for the caller seeded from another member's public creation — the gallery's browse→build loop, and the first real use of `parent_creation_id`.

- **Source authorization** is public-visibility based, not ownership: the source must be `gallery_status='approved'` or `public_status='published'`; owners can remix their own anything. Ineligible sources return the non-enumerating not-found.
- **Privacy contract (hard):** only the rendered artifact HTML seeds the generation (same `prior_html` path as iterate). The source's `prompt_history`, `chat_history`, creator identity, and community NEVER cross over; the remix starts a fresh history containing only the remixer's message. Lineage is stored in `parent_creation_id` but is not exposed on any "who remixed you" surface (that would be a reverse identity leak).
- **Billing/metering:** a remix is a normal build turn — same `gate_builder_or_reason` gate, one-in-flight guard, async `builder_jobs` row (`kind='remix'`, request_type `builder_remix`), one `SURFACE_BUILDER` usage row, raw calls metered under `builder_llm`. No new AI surface.
- The result is a personal creation (`community_id NULL`) the remixer iterates on and shares like any other build. Client entry: Explore's "Build your own" → `/builder?remix=<id>&seed=...`; `useBuilder` routes the session's first build through the remix endpoint, then normal iterate takes over.
