...
- [ ] **Re-request after decline**: owner declines a request; the requester looks the community up again. Expected: no decline notification was sent, the "Ask to join" button is available again (not stuck on "Request sent"), and a fresh request creates a new "Asking to join" card + push for the owner, who can now Accept it.
