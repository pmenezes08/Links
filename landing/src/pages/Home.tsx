  ...
  return (
    <div style={{ position: "relative", height: "100vh", minHeight: 720, overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", left: "var(--rl-gutter)", right: "var(--rl-gutter)", bottom: 72 }}>
        <Eyebrow mb={26}>{pt ? "Redes privadas para organizações" : "Private networks for organisations"}</Eyebrow>
        <h1
          data-reveal=""
        ...
        >
          {pt ? <>A tua organização.<br />Uma rede privada.</> : <>Your organisation.<br />One private network.</>}
        </h1>
        <div data-reveal="" style={{ transitionDelay: ".24s", display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
          <Link to="/platform" className="rl-btn rl-btn--primary">{pt ? "Fala connosco" : "Talk to us"}</Link>
...
function WhatIs({ pt }: { pt: boolean }) {
  ...
  const cols = pt
    ? [
        ...
        { title: "Estruturado como a tua organização", body: "Capítulos, comissões e projetos como subcomunidades sob o mesmo teto — cada uma com o seu feed, calendário e permissões." },
    ...
    : [
        { title: "Private by design", body: "Invitation-only rooms with approval controls. No public feeds, no strangers, no algorithm deciding what your members see." },
        { title: "Structured like your organisation", body: "Chapters, committees, and projects as sub-communities under one umbrella — each with its own feed, calendar, and roles." },
    ...
    <div className="rl-border-t rl-section-pad">
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <p data-reveal="" style={{ transitionDelay: ".2s", margin: "0 0 88px", fontSize: 19, lineHeight: 1.7, color: pt ? "rgba(242,245,244,.74)" : "rgba(242,245,244,.6)", maxWidth: 680 }}>
          {pt
            ? "As organizações usam o C-Point para dar às suas pessoas uma casa dedicada, apenas por convite — com a estrutura de software de comunidades e os hábitos diários de uma app de mensagens, num só sítio que os membros abrem de facto."
            : "Organisations use C-Point to give their people a dedicated, invitation-only home — with the structure of community software and the daily habits of a messaging app, in one place they actually open."}
        </p>
...
function DualPanel({ pt }: { pt: boolean }) {
  return (
    <div className="rl-grid-2 rl-border-t" style={{ minHeight: "88vh" }}>
      ...
      >
        <div data-reveal="">
          <div className="rl-eyebrow" style={{ color: TEAL, marginBottom: 20 }}>{pt ? "01 — Organizações" : "01 — Organisations"}</div>
          <h3 style={{ margin: "0 0 18px", fontWeight: 600, fontSize: "clamp(32px, 5vw, 44px)", lineHeight: 1.04, letterSpacing: "-.02em" }}>
            {pt ? "Para as tuas pessoas" : "For your people"}
          </h3>
          <p style={{ margin: "0 0 36px", fontSize: 16, lineHeight: 1.65, color: pt ? "rgba(238,244,242,.7)" : INK55, maxWidth: 400 }}>
            {pt
              ? "Redes de alumni, clubes, associações, empresas — salas estruturadas, permissões, eventos, e o Steve como presença partilhada."
              : "Alumni networks, clubs, associations, companies — structured rooms, roles, events, and Steve as the shared presence."}
          </p>
          <Link to="/platform" className={pt ? "rl-btn rl-btn--sm rl-btn--primary" : "rl-btn rl-btn--sm rl-btn--ghost"} style={pt ? { padding: "16px 34px" } : undefined}>
      ...
      <div className={pt ? undefined : "rl-dual-right"} style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "64px var(--rl-gutter)", background: pt ? "rgba(242,245,244,.03)" : undefined }}>
        <div data-reveal="" style={{ transitionDelay: ".15s" }}>
          <div className="rl-eyebrow" style={{ color: pt ? "rgba(242,245,244,.55)" : "rgba(242,245,244,.45)", marginBottom: 20 }}>{pt ? "02 — Pessoal" : "02 — Personal"}</div>
          <h3 style={{ margin: "0 0 18px", fontWeight: 600, fontSize: "clamp(32px, 5vw, 44px)", lineHeight: 1.04, letterSpacing: "-.02em" }}>
          ...
          <a href={APP_URL} target="_blank" rel="noopener" className="rl-btn rl-btn--sm rl-btn--ghost" onClick={track("start_community")}>
...
function Why({ pt }: { pt: boolean }) {
  ...
  const rows = pt
    ? [
        ...
        { k: "As redes sociais são donas da tua audiência", v: "No LinkedIn ou nos grupos de Facebook, há um algoritmo entre ti e os teus membros. Aqui a rede é tua — os teus membros, as tuas regras, os teus dados, sem publicidade." },
    ...
    : [
        ...
        { k: "Social networks own your audience", v: "On LinkedIn or Facebook groups, an algorithm sits between you and your members. Here the network is yours — your members, your rules, your data, no ads." },
...
function ProdutoPt() {
  return (
    ...
    >
      ...
      <p data-reveal="" style={{ transitionDelay: ".2s", margin: "0 0 72px", fontSize: 19, lineHeight: 1.6, color: "rgba(242,245,244,.68)", maxWidth: 560 }}>
...
function ProductEn() {
  return (
    ...
    >
      ...
      <p data-reveal="" style={{ transitionDelay: ".2s", margin: "0 0 72px", fontSize: 19, lineHeight: 1.6, color: INK55, maxWidth: 560 }}>
        Every community keeps a real feed — posts, decisions, and plans stay findable, not buried in the scroll.
      </p>
      <Phone src={feedNetwork} alt="C-Point — home timeline across your network" width={380} height={560} openTop delay=".25s" />
    </div>
  );
}
...
export default function Home() {
  ...
  usePageTitle(
    pt
      ? "C-Point | A tua organização. Uma rede privada."
      : "C-Point | Your organisation. One private network.",
  );
